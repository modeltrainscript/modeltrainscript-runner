...
Z21Packet& Z21Translator::canDetector() {
    packet.setHeader(7, Z21_LAN::CAN_DETECTOR);
    packet.setByte(6, 0xD0);
    return packet;
}
...
void Z21Translator::processPacket(uint8_t* data) {
    switch (Z21Packet::getHeader(data)) {
        ...
        case Z21_LAN::RAILCOM_DATACHANGED:
            ...
            break;
        case Z21_LAN::CAN_DETECTOR:
            if (onCanDetector != NULL)
                onCanDetector(Z21Packet::getLEuint16(data, 4), 
                              Z21Packet::getLEuint16(data, 6),
                              Z21Packet::getByte(data, 8),
                              Z21Packet::getByte(data, 9),
